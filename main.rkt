...
(module impl racket
  (provide desugar compile-derp-rules)
...
(require (for-syntax (submod "." impl)))

(define-syntax (derp2-parser stx)
  (syntax-case stx (grammar)
    [(_ clause ...)
     (let ([clauses (syntax->list #'(clause ...))])
       #`(cfg-parser
           #,@(for/list ([clause clauses])
                (syntax-case clause (grammar)
                  [(grammar rule ...)
                   #`(grammar #,@(datum->syntax clause
                                                (compile-derp-rules
                                                  (desugar
                                                    (syntax->datum #'(rule ...))))))]
                  [other #'other]))))]))
